...
router.post('/:listId/share', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/tasklist/:listId/task/:taskId', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
...
router.post('/tasklist', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
...
router.get('/tasklists', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
...
router.patch('/tasklist/:id/permissions', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
...
router.put('/tasklist/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
...
router.delete('/tasklist/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
        ...
        const { id } = req.params;
        const { deleteTasks } = req.query; // Paramètre pour choisir si supprimer les tâches
        ...
        // Supprimer les tâches seulement si demandé
        if (deleteTasks === 'true') {
        ...
        res.json({ message: deleteTasks === 'true' ? 'Task list and associated tasks deleted successfully' : 'Task list deleted successfully, tasks preserved' });
    } catch (error) {
...
router.post('/tasklist/:id/unshare', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
...
router.get('/tasklist/:listId/tasks', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const info = await new Promise((resolve, reject) => {
            jwt.verify(req.cookies.token, env.JWT_SECRET, {}, (err, decoded) => {
                if (err) reject(err);
