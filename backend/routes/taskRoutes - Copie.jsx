...
// Definir une route pour la suppression d'une tâche
router.delete('/task/:id', authMiddleware, async (req, res) => {
  const { id } = req.params;
    ...
    const taskDoc = await Task.findById({_id: id, author: info.userId});
    if (!taskDoc) {
      return res.status(404).json({ message: 'Task not found' });
    }
    
    if (taskDoc.author.toString() !== info.id) {
...
router.get('/tasks/category/:category', authMiddleware, async (req, res) => {
    try {
      ...
      jwt.verify(token, env.JWT_SECRET, {}, (err, decoded) => {
        ...
        const tasks = await Task.find({ category, author: info.userId })
            .populate('author', ['username'])
            .sort({ createdAt: -1 });
        res.json(tasks);
    } catch (error) {
